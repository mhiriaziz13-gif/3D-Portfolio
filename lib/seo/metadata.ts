...
export const createPageMetadata = ({ title, description, path, image = siteSeo.socialImage, noindex = false }: PageMetadata): Metadata => {
  ...
  return {
    ...
    openGraph: { title, description, url: canonical, siteName: siteSeo.siteName, locale: siteSeo.locale, type: "website", images: [{ url: absoluteUrl(image), width: 1200, height: 630, alt: `${title} — Ahmed Aziz Mhiri` }] },
